...
class ReactionModel(FEniCSModel):
    def __init__(self):
        ...
        self.delta = CellDiameter(self.mesh)
    ...
    def stabilization(self, w, u, v):
        return self.delta * (inner(grad(u), w) * inner(grad(v), w) + inner(grad(u), grad(v))) * dx
    ...
    def forward(self, c_prev, w, u, f):
        ...
        L1 += self.dt * f1 * self.v1 * dx
        L2 += self.dt * f2 * self.v2 * dx
        L3 += self.dt * f3 * self.v3 * dx
